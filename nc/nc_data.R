...
make_nc_df = function(county="Dare") {
  ...
  voters = voters_raw %>%
    ...
    select(regnum=voter_reg_num,
           ...
           address, city, zip=zip_code, county_name,
           race, age, party, lic) %>%
...
data <- make_nc_statewide("./nc_voter_data.rds")
...
#   d_raw = map_dfr(tables, ~ tidycensus::get_decennial("zcta", year=2010, table=., state="NC"))
#   d = d_raw %>%
#     select(-NAME) %>%
